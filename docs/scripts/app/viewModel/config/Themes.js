define([ "require", "jquery", "lodash", "footwork", "LoadState" ],
  function( require, $, _, ko, LoadState ) {
    var themes;
    var $head = $('head');
    var $chosenStyle;
    var themePicker = {};
    var disableModifiedCheck = false;
    var lessMapping = {
    ...
    var themesNamespace = ko.namespace('Themes');
    var processingLESS = ko.observable().extend({ throttle: { timeout: 50, when: function(isProcessing) { return isProcessing === false; } } });
    var updateLESSTimeout;
    var updateLESS = function() {
    ...
    return ko.viewModel({
      ...
      initialize: function() {
          ...
          if( currentTab === 'customize' ) {
            if( $.fn.spectrum === undefined ) {
              ...
              require(["jquery.spectrum"], function() {
                var $spectrumCSS;
                var $colorPickers = $('.js-theme-color-input');
                ...
                $spectrumCSS.load(function() {
                  ...
                  this.configVisible.subscribe(function(state) { state === false && $colorPickers.spectrum('hide'); });
        ...
        this.currentTheme.subscribe(function(theme) {
          if( this.configVisible() === true ) {
            var filePartial = encodeURIComponent(theme.filePartial());
            var stylesheetURL = 'css/builds/build-' + filePartial + '.css';
            var faviconURL = 'images/icon-builds/favicon-' + filePartial + '.png';
        ...
        this.configVisible.subscribe(function(visible) {
        ...
        this.chooseColor = function(model, event) {
          ...
          var propName = $target.data('theme-property');
          setTimeout(function() {
